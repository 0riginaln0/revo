...
pub fn printDisassembly(vm: *revo.VM, bytecode: revo.lang.Bytecode, source: []const u8) !void {
    const insts = bytecode.instructions;
    const spans = bytecode.spans;
    const n = insts.len;
    if (n == 0) return;
    ...
    for (insts, 0..) |inst, pc| {
        switch (inst.op) {
            ...
            .make_closure => {
                const template_id = inst.bx;
                if (template_id < vm.callable.templates.items.len) {
                    const template = vm.callable.templates.items[template_id];
                    if (template.addr < n and template.addr != 0) try entries.put(template.addr, template.name);
                }
            },
            else => {},
        }
...
fn dataText(vm: *revo.VM, val: revo.Value, buf: []u8) []const u8 {
    if (val.asNumOpt()) |n| {
        if (n == @trunc(n)) {
...
/// collapsed, truncated text of src span an instruction was compiled from
...
fn operandText(vm: *revo.VM, inst: revo.Instruction, buf: []u8) []const u8 {
    ...
    switch (op) {
        ...
        .store_user_global, .store_user_global_const => {
            const name = vm.stringValue(bx);
            if (std.mem.eql(u8, name, "<dead>")) return fmt(buf, "?, r{d}", .{a});
            return fmt(buf, ":{s}, r{d}", .{ name, a });
        },
        .load_builtin_global => return fmt(buf, "r{d}, global#{d}", .{ a, bx }),
        ...
        .make_closure => {
            if (bx < vm.callable.templates.items.len) {
                const name = vm.callable.templates.items[bx].name;
                if (name.len > 0) return fmt(buf, "r{d}, template#{d} ({s})", .{ a, bx, name });
            }
            return fmt(buf, "r{d}, template#{d}", .{ a, bx });
        },
