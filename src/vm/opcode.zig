...
pub const Opcode = enum(u8) {
    ...
    load_user_global, // "R[a] <- globals[bx]"
    load_builtin_global, // "R[a] <- builtin_globals[bx]"
    store_user_global, // "globals[bx] <- R[a]"
    store_user_global_const, // "globals[bx] <- R[a], mark const"
    ...
    make_closure, // "R[a] <- closure(template=bx)"
    add, // "R[a] <- R[b] + R[c]"
    concat, // "R[a] <- R[b] ~ R[c]"
    sub, // "R[a] <- R[b] - R[c]"
    mul, // "R[a] <- R[b] * R[c]"
