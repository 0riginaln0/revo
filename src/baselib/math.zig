...
pub const Impl = struct {
    ...
    pub fn @"is_close?"(vm: *VM, x: Args.number, y: Args.number, places: Args.number) !HostResult {
        _ = vm;
...
test "math library" {
    ...
    try testing.topTrue("math.is_close?(1.5, 1.5004, 3)");
    try testing.topFalse("math.is_close?(1.5, 1.5004, 4)");
}
