...
const TRANSLATOR = [_]TypeTranslation{
    ...
    .{ .zig = "?*ErevoValue", .c = "ErevoValue*" },
    .{ .zig = "Value", .c = "RevoValue" },
    .{ .zig = "*Value", .c = "RevoValue*" },
    .{ .zig = "[*]const Value", .c = "const RevoValue*" },
    .{ .zig = "ErevoValue", .c = "RevoValue" },
};
...
pub fn data(allocator: Allocator) !std.ArrayList(u8) {
    ...
    inline for (.{ "exports.zig", "embed.zig" }) |mod| {
        const source = @embedFile(mod);
        const source_z = try allocator.dupeSentinel(u8, source, 0);
        defer allocator.free(source_z);

        const category: Function.Category = if (std.mem.endsWith(u8, mod, "exports.zig")) .revo else .erevo;
    ...
    try header.appendSlice(allocator,
        ...
        \\#define revo_string(id) ((RevoValue)(REVO_BOX_TAG | ((uint64_t)revo_string << REVO_TAG_SHIFT) | (id)))
        \\#define revo_table(id) ((RevoValue)(REVO_BOX_TAG | ((uint64_t)revo_table << REVO_TAG_SHIFT) | (id)))
        \\#define revo_function(id) ((RevoValue)(REVO_BOX_TAG | ((uint64_t)revo_function << REVO_TAG_SHIFT) | (id)))
        ...
        \\}
        \\static inline RevoValue revo_num(double n) { union { uint64_t u; double f; } u = { .f = n }; return u.u; }
        \\static inline RevoValue revo_atom_val(uint64_t id) { return (RevoValue)(REVO_BOX_TAG | ((uint64_t)revo_atom << REVO_TAG_SHIFT) | id); }
        \\static inline double revo_num_value(RevoValue d) { union { uint64_t u; double f; } u = { .u = d }; return u.f; }
        ...
        \\static inline int revo_is_bool(RevoValue d) { return revo_is_atom(d) && ((d & REVO_PAYLOAD_MASK) == ra_true || (d & REVO_PAYLOAD_MASK) == ra_false); }
        \\static inline int revo_bool_val(RevoValue d) { return revo_is_bool(d) ? ((d & REVO_PAYLOAD_MASK) == ra_true ? 1 : 0) : 0; }
        \\
        \\// function ptr type; returns REVO_OK (0), anything else raises
        \\// (`*out` used on ok only, ignored on err)
        \\typedef int (*RevoFn)(void *vm, size_t argc, RevoValue *argv, RevoValue *out_result);
        \\
...
fn buildSignature(
    ...
    if (params.items.len == 0) {
        ...
        for (params.items, 0..) |param, i| {
            // for bool revo_table_get(void* vm, RevoValue table, RevoValue key, RevoValue* out);,
            ...
            // (void* vm, RevoValue table, RevoValue key, RevoValue* out);
